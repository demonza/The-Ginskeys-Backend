...
// FIX: use rounds=10 in seed (12 is fine for prod login but makes seed extremely slow)
...
async function seed() {
  ...
  try {
    ...
    // ── Admin user ──────────────────────────────────
    const adminPassword = process.env.ADMIN_PASSWORD || 'GK!Admin2026';
    ...
    const defaultPass = await bcrypt.hash('GK!Change2026', SALT_ROUNDS);
    for (const u of otherUsers) {
      await client.query(`
        INSERT INTO users (id, email, name, role, password_hash, active)
        VALUES ($1,$2,$3,$4,$5,true)
        ON CONFLICT (email) DO NOTHING
      `, [uuid(), u.email, u.name, u.role, defaultPass]);
    }

    // ── Tours ───────────────────────────────────────
    // FIX: check existence by name before inserting to avoid duplicates on re-run
    const TOURS = [
      { name:'Concertos 2022',             start:'2022-06-10', end:'2022-10-21', budget:200,  status:'completed' },
      { name:'Tasquinhas 2023',            start:'2023-08-01', end:'2023-08-01', budget:100,  status:'completed' },
      { name:'Semana da Juventude 2024',   start:'2024-04-01', end:'2024-04-01', budget:100,  status:'completed' },
      { name:'Abril em Odemira 2024',      start:'2024-06-01', end:'2024-06-01', budget:100,  status:'completed' },
    ];
    const tourMap = {};
    for (const t of TOURS) {
      const { rows: existing } = await client.query(
        'SELECT id FROM tours WHERE name = $1', [t.name]
      );
      let tourId;
      if (existing.length) {
        ...
        `, [uuid(), t.name, t.start, t.end, t.budget, t.status]);
        tourId = inserted[0].id;
      }
      tourMap[t.name] = tourId;
    }
    ...
    if (parseInt(existing[0].count) > 0) {
      ...
      for (const t of sorted) {
        ...
        `, [uuid(), t.date, t.type, catId, t.amount, t.desc, t.tags, tourId, realAdminId]);
      }
