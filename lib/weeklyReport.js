...
async function generateWeeklySynthesisPDF(preGathered = null) {
  const d = preGathered || await gatherWeekData();
...
module.exports = { generateWeeklySynthesisPDF, gatherWeekData, buildEmailHtml };

// ══════════════════════════════════════════════════════════════════
// EMAIL BODY (Portuguese) — separate from the PDF's layout, this is
// what the band actually reads first, in the inbox itself. Every
// number and every recommendation below is pulled from the same `d`
// object the PDF uses — nothing here is generated freely. The
// "recommendations" are fixed if/then rules over real fields
// (overdue follow-ups, runway, pipeline value, week's activity), not
// AI-written commentary — so what you read is exactly reproducible
// from the numbers next to it.
// ══════════════════════════════════════════════════════════════════
function buildEmailHtml(d) {
  const eur = n => Number(n || 0).toLocaleString('pt-PT', { style: 'currency', currency: 'EUR' });
  const dateStr = dt => new Date(dt).toLocaleDateString('pt-PT');

  // ── Headline (same logic as the PDF, in PT) ──
  let headline;
  if (d.weekRevenue > 0) {
    headline = `Entraram ${eur(d.weekRevenue)} esta semana — a tesouraria verificada está agora em ${eur(d.bandCash)}.`;
  } else if (d.stageMoves.length > 0) {
    headline = `${d.stageMoves.length} contacto${d.stageMoves.length === 1 ? '' : 's'} de booking mudou de fase esta semana.`;
  } else {
    headline = `Sem receita nova nem movimento na pipeline esta semana — tesouraria verificada mantém-se em ${eur(d.bandCash)}.`;
  }

  // ── Recommendations — deterministic rules over real fields ──
  const recs = [];

  if (d.overdueFollowups > 0) {
    recs.push(`Há <strong>${d.overdueFollowups} contacto${d.overdueFollowups === 1 ? '' : 's'} de booking</strong> com follow-up em atraso. Vale a pena retomar contacto esta semana antes que arrefeçam mais.`);
  }

  if (d.pipeline.items.length > 0) {
    const top = d.pipeline.items.reduce((best, i) => i.expected_value_eur > (best?.expected_value_eur || 0) ? i : best, null);
    if (top) {
      recs.push(`O contacto com maior valor esperado na pipeline é <strong>${top.name}</strong> (fase: ${top.stage}, fee: ${eur(top.fee_eur)}, valor esperado: ${eur(top.expected_value_eur)}${top.probability_source === 'prior' ? ' — probabilidade ainda estimada, não medida' : ''}).`);
    }
  } else {
    recs.push(`Não há bookings em aberto na pipeline neste momento — pode ser boa altura para reforçar o outreach.`);
  }

  if (d.runway.runway_months_cash_only !== null) {
    if (d.runway.runway_months_cash_only < 5) {
      recs.push(`A autonomia de tesouraria (só com o cash atual, sem contar pipeline) está em <strong>${d.runway.runway_months_cash_only.toFixed(1)} meses</strong> — abaixo do que seria confortável. Vale a pena vigiar de perto.`);
    }
  } else {
    recs.push(`Não há despesas registadas nos últimos meses suficientes para calcular a autonomia de tesouraria com fiabilidade.`);
  }

  if (d.weekRevenue === 0 && d.stageMoves.length === 0 && d.releasesActivity.length === 0 && d.pressActivity.length === 0) {
    recs.push(`Semana sem qualquer movimento registado (receita, bookings, releases ou imprensa). Se isto se repetir, pode valer a pena rever prioridades.`);
  }

  const recsHtml = recs.length
    ? `<ul style="padding-left:18px;margin:8px 0;">${recs.map(r => `<li style="margin-bottom:8px;">${r}</li>`).join('')}</ul>`
    : `<p style="color:#666;">Sem recomendações específicas esta semana — tudo dentro do esperado.</p>`;

  return `
  <div style="font-family:Helvetica,Arial,sans-serif;max-width:600px;margin:0 auto;color:#1a1a1a;">
    <h2 style="color:#d64533;margin-bottom:4px;">THE GINSKEYS</h2>
    <p style="color:#888;font-size:13px;margin-top:0;">Síntese semanal — semana de ${dateStr(d.since)} a ${dateStr(new Date())}</p>

    <p style="font-size:16px;font-weight:bold;margin:20px 0;">${headline}</p>

    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
      <tr>
        <td style="padding:8px 0;border-bottom:1px solid #eee;">Receita esta semana</td>
        <td style="padding:8px 0;border-bottom:1px solid #eee;text-align:right;font-weight:bold;color:#2a7d2a;">${eur(d.weekRevenue)}</td>
      </tr>
      <tr>
        <td style="padding:8px 0;border-bottom:1px solid #eee;">Despesas esta semana</td>
        <td style="padding:8px 0;border-bottom:1px solid #eee;text-align:right;font-weight:bold;color:#b03030;">${eur(d.weekExpense)}</td>
      </tr>
      <tr>
        <td style="padding:8px 0;border-bottom:1px solid #eee;">Tesouraria verificada (Trust Engine)</td>
        <td style="padding:8px 0;border-bottom:1px solid #eee;text-align:right;font-weight:bold;">${eur(d.bandCash)}</td>
      </tr>
      <tr>
        <td style="padding:8px 0;">Pipeline (valor esperado)</td>
        <td style="padding:8px 0;text-align:right;font-weight:bold;color:#2a5a9a;">${eur(d.pipeline.total_expected_value_eur)}</td>
      </tr>
    </table>

    <h3 style="border-bottom:2px solid #1a1a1a;padding-bottom:4px;">Recomendações</h3>
    ${recsHtml}

    <p style="margin-top:24px;color:#555;">O relatório completo em PDF, com a pipeline detalhada e o movimento da semana, está em anexo.</p>

    <p style="color:#999;font-size:11px;margin-top:32px;border-top:1px solid #eee;padding-top:12px;">
      Gerado automaticamente pela Ginskeys Console. Os valores financeiros são verificados contra o ledger hash-chained do Trust Engine.
    </p>
  </div>`;
}
